...
def run_go_test(go_file, port):
    # Form the ldflags argument with the provided port
    ldflags = f'-X "main.BaseIPAddress=http://localhost:{port}/api/v1"'

    # Open test.log file for writing logs
    with open("test.log", "w") as log_file:
        # Run the Go file with the ldflags argument
        result = subprocess.run(
            ['go', 'run', '-ldflags', ldflags, go_file], 
            capture_output=True, 
            text=True
        )
        ...
        # Print stdout and stderr to the console for debugging
        print("STDOUT:")
        print(stdout)
        print("\nSTDERR:")
        print(stderr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("port", help="Port to be used in the ldflags argument.")
    
    args = parser.parse_args()
    run_go_test(args.go_file, args.port)
